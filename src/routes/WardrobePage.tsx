    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="p-6 md:p-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl font-black text-zinc-900 dark:text-zinc-50">Fashion Wardrobe & Date Simulator</h1>
            ...
            {beauties.map((beauty) => (
              ...
              >
                <div className="flex items-center justify-between w-full">
                  ...
                  <span className="text-[10px] font-mono text-zinc-400">ID: {beauty.id}</span>
                </div>
                <div className="flex items-center gap-1.5 text-xs text-zinc-455 font-mono">
                  <Calendar size={12} />
            ...
            <div className="space-y-6">
              ...
              <div className="p-6 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-100 dark:border-zinc-800 pb-3">
                  <div>
                    <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-widest block">ACTIVE MODEL PROFILE</span>
                    <h2 className="text-2xl font-black text-zinc-850 dark:text-zinc-50">{selectedBeauty.name}</h2>
                  </div>
                ...
                <div className="p-4 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-900 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    {selectedBeauty.rewards.map((rew, idx) => (
                      <div key={idx} className="bg-white dark:bg-zinc-900 px-3 py-2 rounded-lg border border-zinc-150 dark:border-zinc-800/80 flex items-center justify-between">
                        <span className="text-zinc-400 font-semibold">Reward Code: {rew.code}</span>
                        <span className="font-mono font-bold text-zinc-800 dark:text-zinc-250">Reward Type: #{rew.type}</span>
                      </div>
                    ))}
                    {selectedBeauty.rewards.length === 0 && (
                      <span className="text-zinc-450 italic">No static rewards defined.</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <h3 className="font-extrabold text-base text-zinc-850 dark:text-zinc-150 flex items-center gap-2 px-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {selectedBeautyStages.map((stage, idx) => {
                    ...
                    return (
                      <div
                        key={stage.id}
                        className="p-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm flex flex-col justify-between space-y-4 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all"
                      >
                        <div className="space-y-2">
                          <div className="flex items-center justify-between w-full border-b border-zinc-100 dark:border-zinc-800 pb-2">
                            ...
                            <span className="font-mono text-[9px] text-zinc-400">ID: {stage.id}</span>
                          </div>
                          
                          {/* Question */}
                          <div className="space-y-1">
                            <span className="text-[10px] uppercase font-bold text-zinc-400 block">Dialogue Trigger / Prompt</span>
                        ...
                        {stage.answers.length > 0 ? (
                          <div className="space-y-2">
                            <span className="text-[10px] uppercase font-bold text-zinc-450 block">Answers & Intimacy Gain</span>
                                  ...
                                  <div
                                    ...
                                    className={`p-2 rounded-lg border transition-all flex items-center justify-between ${
                                      isBest
                                        ...
                                        : 'border-zinc-100 dark:border-zinc-850/60 text-zinc-650 dark:text-zinc-350'
                                    }`}
                                  >
                                    <div className="flex items-center gap-1.5 truncate">
                                      {isBest ? <CheckCircle2 size={14} className="text-emerald-500 shrink-0" /> : <HelpCircle size={14} className="text-zinc-400 shrink-0" />}
                                      <span className="truncate">{answer.replace(/^\d+:/, '')}</span>
                                    </div>
                                    <span className={`font-mono text-xs font-bold shrink-0 ${isBest ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-400'}`}>
                                      +{score} Pts
                                    </span>
                                  </div>
                                );
                          ...
                          <div className="p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-900 text-xs text-zinc-450 italic text-center">
