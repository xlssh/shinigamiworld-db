    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="p-6 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm flex flex-col md:flex-row gap-6 items-start">
        <div className="flex-1 space-y-4">
          ...
          {quest.description && (
            <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-950/40 border border-zinc-200/40 dark:border-zinc-800">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-zinc-400">Quest Briefing Instructions</span>
              <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed font-semibold italic">
                "{quest.description}"
              </p>
            </div>
          )}
          ...
          {quest.instant_json && (
            <div className="p-4 rounded-xl border border-zinc-100 dark:border-zinc-800/80 bg-zinc-50/30 dark:bg-zinc-950/10 space-y-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1">
                <Target size={14} className="text-orange-500" />
                <span>Completion Target Parameters</span>
              </span>
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 p-3 rounded-lg flex flex-wrap items-center gap-6 text-xs md:text-sm">
                <div>
                  <span className="text-zinc-400 block mb-0.5 font-semibold">Target Action type</span>
                ...
                <div>
                  <span className="text-zinc-400 block mb-0.5 font-semibold">Required Execution Count</span>
                  <span className="font-mono font-bold text-zinc-700 dark:text-zinc-200">
        ...
        <div className="w-full md:w-64 border border-zinc-100 dark:border-zinc-800 rounded-xl p-4 bg-zinc-50/50 dark:bg-zinc-950/20 space-y-3 shrink-0">
          <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-400 border-b border-zinc-200 dark:border-zinc-800 pb-1.5">Task Properties</h4>
          <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-xs">
            <div>
              <span className="text-zinc-400 block mb-0.5">Task Type</span>
              <span className="font-semibold text-zinc-700 dark:text-zinc-350">Type {quest.type}</span>
            </div>
            <div>
              <span className="text-zinc-400 block mb-0.5">Event Type</span>
              <span className="font-semibold text-zinc-700 dark:text-zinc-350">Action {quest.event_type}</span>
            </div>
            <div>
              <span className="text-zinc-400 block mb-0.5">Activity Points</span>
              <span className="font-bold text-emerald-600 font-mono">+{quest.point ?? 0}</span>
            </div>
            <div>
              <span className="text-zinc-400 block mb-0.5">Weight Rate</span>
              <span className="font-semibold text-zinc-700 dark:text-zinc-350 font-mono">{quest.rate ?? 1}</span>
            </div>
            <div>
              <span className="text-zinc-400 block mb-0.5">Allow Cancel</span>
              <span className="font-semibold text-zinc-700 dark:text-zinc-350">{quest.cancel === 1 ? 'Yes' : 'No'}</span>
            </div>
            <div>
              <span className="text-zinc-400 block mb-0.5">Instant Teleport</span>
              <span className="font-semibold text-zinc-700 dark:text-zinc-350">{quest.isgoto === 1 ? 'Yes' : 'No'}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-xl shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-mono">
          <div>
            <span className="text-zinc-400 block mb-0.5 font-sans font-semibold">Pre-requisite Complete ID</span>
            <span className="font-semibold text-zinc-700 dark:text-zinc-300">ID #{quest.complete ?? 0}</span>
          </div>
          <div>
            <span className="text-zinc-400 block mb-0.5 font-sans font-semibold">Small Sprite ID</span>
            <span className="font-semibold text-zinc-700 dark:text-zinc-300">Sprite #{quest.small_picture ?? 0}</span>
          </div>
        </div>
      </div>
