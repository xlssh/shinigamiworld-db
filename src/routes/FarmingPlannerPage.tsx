    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="p-6 md:p-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-zinc-900 dark:text-zinc-50">Loot Finder & Farming Planner</h1>
            <p className="text-xs text-zinc-500 font-semibold">Select or search an item to discover all drop stages, cash shops, and quests rewarding it.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            {filteredArticles.map(a => (
              ...
              >
                <div className="truncate pr-2">
                  ...
                  <span className="text-[10px] text-zinc-400">{getMinorTypeLabel(a.major_type, a.minor_type)}</span>
                </div>
                <span className="font-mono text-[9px] text-zinc-400 shrink-0">#{a.id}</span>
              </button>
            ))}
            {filteredArticles.length === 0 && (
              <p className="text-xs text-zinc-400 text-center py-8">No matching items found.</p>
            )}
          </div>
        </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          {selectedArticle && (
            <div className="p-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-zinc-100 dark:border-zinc-800">
                <div>
                  <span className="text-[10px] font-mono text-zinc-400 block font-bold">FOCUS TARGET</span>
                  <h2 className="text-xl font-black text-zinc-900 dark:text-zinc-50">{selectedArticle.name}</h2>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Drop Stages */}
            <div className="p-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm space-y-4">
              <h3 className="font-extrabold text-sm text-rose-600 dark:text-rose-450 flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-2">
                <Swords size={16} />
                ...
                <div className="space-y-3">
                  {farmingSources.stages.map(stage => (
                    <Link
                      key={stage.id}
                      to={`/stages/${stage.id}`}
                      className="p-3 border border-zinc-100 dark:border-zinc-800 rounded-xl block hover:border-rose-500 hover:bg-rose-500/5 transition-all text-xs"
                    >
                      <span className="font-bold text-zinc-800 dark:text-zinc-200 block truncate">{stage.name}</span>
                      <span className="text-[10px] text-zinc-400 block mt-0.5">Req Level: Lv. {stage.level} | Hardness: {stage.hard}</span>
                    </Link>
                  ))}
                </div>
              ) : (
                <div className="text-xs text-zinc-400 italic py-4 flex items-center gap-1.5 bg-zinc-50/50 dark:bg-zinc-950/20 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800/80">
                  <AlertCircle size={14} />
            ...
            {/* Quests */}
            <div className="p-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm space-y-4">
              <h3 className="font-extrabold text-sm text-indigo-600 dark:text-indigo-400 flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-2">
                <Award size={16} />
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                  {farmingSources.quests.story.map(q => (
                    <Link
                      ...
                      className="p-3 border border-zinc-100 dark:border-zinc-800 rounded-xl block hover:border-indigo-500 hover:bg-indigo-500/5 transition-all text-xs"
                    >
                      <div className="flex items-center justify-between gap-1">
                        <span className="font-bold text-zinc-800 dark:text-zinc-200 block truncate">{q.name}</span>
                  ...
                  {farmingSources.quests.daily.map(q => (
                    <Link
                      ...
                      className="p-3 border border-zinc-100 dark:border-zinc-800 rounded-xl block hover:border-orange-500 hover:bg-orange-500/5 transition-all text-xs"
                    >
                      <div className="flex items-center justify-between gap-1">
                        <span className="font-bold text-zinc-800 dark:text-zinc-200 block truncate">{q.task_name}</span>
            ...
            {/* Mall Items */}
            <div className="p-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm space-y-4">
              <h3 className="font-extrabold text-sm text-violet-600 dark:text-violet-450 flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-2">
                <ShoppingBag size={16} />
                ...
                <div className="space-y-3">
                  {farmingSources.shops.map(item => (
                    <Link
                      ...
                      className="p-3 border border-zinc-100 dark:border-zinc-800 rounded-xl block hover:border-violet-500 hover:bg-violet-500/5 transition-all text-xs"
                    >
                      <span className="font-bold text-zinc-800 dark:text-zinc-200 block truncate">{item.name}</span>
