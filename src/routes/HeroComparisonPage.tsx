...
export const HeroComparisonPage: React.FC = () => {
  ...
  const compareValues = (v1: number | undefined | null, v2: number | undefined | null, higherIsBetter = true) => {
    const val1 = v1 ?? 0;
    const val2 = v2 ?? 0;
    if (val1 === val2) return { textClass: 'text-zinc-500 dark:text-zinc-400', suffix: '' };
    if (higherIsBetter) {
      return val1 > val2
  ...
  if (heroes.length < 2) {
    return (
      <div className="p-8 text-center bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 space-y-4">
        <AlertCircle className="mx-auto text-amber-500" size={48} />
        <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Insufficient Data</h3>
        <p className="text-sm text-zinc-500">Requires at least 2 characters in the local cache to execute a comparison matrix.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="p-6 md:p-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-zinc-900 dark:text-zinc-50">Mercenary Comparison Matrix</h1>
            <p className="text-xs text-zinc-500">Side-by-side growth analytics, secondary rates, and tactical scaling coefficients.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Selector Card 1 */}
        <div className="p-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm space-y-4 relative">
          <span className="absolute top-4 right-4 text-[9px] font-bold text-zinc-400 uppercase tracking-widest">Subject Alpha</span>
          <label className="block text-xs font-bold uppercase tracking-wider text-zinc-400">Select Character A</label>
          <div className="relative">
            <select
              value={hero1Id}
              onChange={(e) => setHero1Id(parseInt(e.target.value))}
              className="w-full pl-3 pr-10 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 focus:outline-none focus:ring-1.5 focus:ring-violet-500 font-bold text-zinc-850 dark:text-zinc-100 appearance-none cursor-pointer"
            >
              {heroes.map(h => (
          ...
          {hero1 && (
            <div className="flex flex-wrap items-center gap-2.5 pt-2">
              ...
              <span className="px-2 py-0.5 text-[10px] font-bold bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded">
        ...
        {/* Selector Card 2 */}
        <div className="p-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm space-y-4 relative">
          <span className="absolute top-4 right-4 text-[9px] font-bold text-zinc-400 uppercase tracking-widest">Subject Beta</span>
          <label className="block text-xs font-bold uppercase tracking-wider text-zinc-400">Select Character B</label>
          <div className="relative">
            <select
              value={hero2Id}
      ...
      {hero1 && hero2 && (
        <div className="grid grid-cols-1 gap-6">
          ...
          <div className="border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/20 flex items-center gap-2">
              <Swords size={18} className="text-violet-500" />
              <h3 className="font-bold text-sm text-zinc-850 dark:text-zinc-100">Base Attribute Comparisons</h3>
            </div>
                ...
                return (
                  <div key={m.key} className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-3 items-center gap-4 text-xs">
                    ...
                    <div className="space-y-1">
                      <span className="font-bold text-zinc-800 dark:text-zinc-200 text-sm block">{m.label}</span>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between font-mono">
                        <span className="text-zinc-500 font-semibold">{hero1.name}</span>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between font-mono">
                        <span className="text-zinc-500 font-semibold">{hero2.name}</span>
          ...
          <div className="border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/20 flex items-center gap-2">
              ...
              <h3 className="font-bold text-sm text-zinc-850 dark:text-zinc-100">Growth Rate Matrix</h3>
            </div>
            <div className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
              {growthMetrics.map(m => {
                const val1 = (hero1 as any)[m.key] as number;
                ...
                return (
                  <div key={m.key} className="p-4 grid grid-cols-3 text-center text-xs font-mono">
                    <span className="font-sans font-bold text-zinc-500 text-left pl-2">{m.label}</span>
          ...
          <div className="border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/20 flex items-center gap-2">
              ...
              <h3 className="font-bold text-sm text-zinc-850 dark:text-zinc-100">Combat Trigger Probabilities (Secondary Rates)</h3>
            </div>
            <div className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
              {rateMetrics.map(m => {
                const val1 = (hero1 as any)[m.key] as number;
