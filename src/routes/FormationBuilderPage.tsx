...
const SlotCard: React.FC<SlotCardProps> = ({ slot, col, hero, isSelected, isActive, isMc, onClick, onRemove }) => {
  ...
  return (
    <div onClick={onClick}
      ...
      {!isActive && (
        <div className="absolute inset-0 rounded-xl flex items-center justify-center bg-zinc-950/50">
          <span className="text-[8px] font-bold text-amber-400 bg-zinc-900 px-1.5 py-0.5 rounded">Benched</span>
        </div>
      )}
      <div className="flex items-center justify-between">
        <span className="text-[8px] text-zinc-600">#{slot} {isMc && <span className="text-[8px] font-bold text-fuchsia-400">★ Main</span>}</span>
      ...
      <div className="flex items-center gap-1">
        <span className={`text-[8px] font-bold ${PROF_COLOR[hero.profession ?? 0] ?? 'text-zinc-500'}`}>{getProfessionLabel(hero.profession)}</span>
        {hero.role && <span className="text-[8px] text-zinc-500">{hero.role}</span>}
      </div>
    </div>
  );
};
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">⚔️ Formation Builder</h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={clearAll} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-zinc-800 hover:bg-rose-950/40 text-zinc-400 hover:text-rose-400 rounded-lg border border-zinc-700 transition-colors">
      ...
      {/* Level slider */}
      <div className="flex items-center gap-4 p-3 bg-zinc-900/70 rounded-xl border border-zinc-800">
        <TrendingUp size={14} className="text-fuchsia-400 shrink-0" />
        <span className="text-xs font-bold text-zinc-400 w-20 shrink-0">Sim Level</span>
      ...
      <div className="p-4 bg-zinc-900 border border-zinc-800 rounded-xl flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        <div className="space-y-1">
          <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider block">Wielder (Main Character)</span>
          <span className="text-[11px] text-zinc-500">Every team deployment requires exactly one Main Character. Select class & grid position.</span>
        </div>
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative flex-1 sm:flex-initial">
            <select
              ...
              className="px-3 py-2 border border-zinc-800 bg-zinc-950 text-xs font-bold rounded-xl text-fuchsia-300 w-full cursor-pointer"
            >
              {mcList.map(h => (
          ...
          {/* Position Selector */}
          <div className="flex items-center gap-1.5 bg-zinc-950 p-1.5 border border-zinc-800 rounded-xl">
            <span className="text-[10px] font-bold text-zinc-500 uppercase px-1.5">Grid Slot:</span>
      ...
      {tab === 'build' && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 space-y-5">
            ...
            <div className={`flex items-center gap-3 p-3 rounded-xl border ${finalActiveSet.size >= MAX_PARTNERS ? 'border-amber-700/50 bg-amber-950/20' : 'border-zinc-800 bg-zinc-900/50'}`}>
              <Users size={14} className={finalActiveSet.size >= MAX_PARTNERS ? 'text-amber-400' : 'text-fuchsia-400'} />
              <span className="text-xs font-bold text-zinc-400">Active Partners:</span>
              <div className="flex gap-1">
              ...
              {COLUMNS.map((col, ci) => (
                <div key={col.id} className="flex-1 space-y-2 min-w-0">
                  <div className={`text-center text-[10px] font-black uppercase tracking-widest ${col.headerColor}`}>
                    {col.label}
                    <span className="block text-[8px] text-zinc-600 font-normal normal-case tracking-normal">{col.profLabel}</span>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-zinc-900 border border-zinc-800 rounded-2xl space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black uppercase tracking-wider text-zinc-400">Power</span>
                </div>
              ...
              <div className="p-4 bg-zinc-900 border border-zinc-800 rounded-2xl space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black uppercase tracking-wider text-zinc-400">Range Coverage</span>
                </div>
                {[
                ...
                {[0,1,2].map(i => (
                  <div key={i} className="space-y-1.5">
                    ...
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => loadPreset(i)} disabled={!presetIds[i]} className="flex-1 py-1.5 text-[9px] font-bold bg-zinc-800 hover:bg-amber-900/30 text-zinc-400 hover:text-amber-300 rounded-lg border border-zinc-700 disabled:opacity-30 transition-colors">Load</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-4">
            {selectedSlot && selColMeta ? (
              <div className={`p-3 rounded-xl border-2 ${selColMeta.selBorder} bg-zinc-900 space-y-2`}>
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setSelectedSlot(null)} className="text-zinc-600 hover:text-zinc-400"><X size={12} /></button>
                </div>
                {slotSuggestions.length > 0 && (
                  <div className="space-y-1">
                    <span className="text-[9px] font-bold text-zinc-500 uppercase">Recommended ({selColMeta.profLabel}):</span>
                    {slotSuggestions.map(h => (
                      <button key={h.id} onClick={() => placeHero(h)} className="w-full flex items-center justify-between p-1.5 rounded-lg bg-zinc-800 hover:bg-fuchsia-900/30 border border-zinc-700 hover:border-fuchsia-600 transition-colors">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[9px] text-zinc-500">{h.role}</span>
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <div className="p-3 rounded-xl border border-zinc-800 bg-zinc-900 text-center text-xs text-zinc-600">👈 Click a grid slot, then pick a hero</div>
            )}
            ...
            <input type="text" placeholder="Search hero..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              className="w-full px-3 py-2 text-xs bg-zinc-900 border border-zinc-800 rounded-xl text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-fuchsia-700 transition-colors" />

            <div className="space-y-1.5 max-h-[440px] overflow-y-auto">
              {heroPool.length === 0 && <div className="text-center text-xs text-zinc-600 py-8">No heroes</div>}
              {heroPool.map(hero => {
                ...
                return (
                  <button key={hero.id} onClick={() => selectedSlot ? placeHero(hero) : undefined}
                    className={`w-full flex items-center justify-between p-2.5 rounded-xl border text-left transition-all ${selectedSlot ? 'border-zinc-800 bg-zinc-900 hover:border-fuchsia-600 hover:bg-fuchsia-950/20 cursor-pointer' : 'border-zinc-800 bg-zinc-900 opacity-60 cursor-default'}`}>
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="flex items-center gap-1">
                        <span className={`text-[9px] font-bold ${PROF_COLOR[hero.profession ?? 0] ?? 'text-zinc-500'}`}>{getProfessionLabel(hero.profession)}</span>
                        {rec?.ability && <span className="text-[9px] text-zinc-600">· {rec.ability}</span>}
                      </div>
                    </div>
                    <div className="text-right shrink-0 pl-2 space-y-0.5">
                      ...
                      <div className="text-[9px] font-mono text-zinc-500">{power.toLocaleString()}</div>
                    </div>
                  </button>
                );
              })}
      ...
      {tab === 'export' && (
        <div className="space-y-5">
          <p className="text-sm text-zinc-400">Export or share your current formation.</p>
          {/* Formation Card */}
          <div id="formation-export-card" className="p-6 bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-700 rounded-2xl space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-zinc-500 mt-0.5">{activeSet.size}/{MAX_PARTNERS} active · Lv.{targetLevel} simulation</div>
              </div>
              <div className="text-right">
                <div className="text-xl font-black text-fuchsia-300 font-mono">{totalPower.toLocaleString()}</div>
                <div className="text-[9px] text-zinc-500">total power</div>
              </div>
            </div>
            <div className="flex gap-4">
              {COLUMNS.map((col, ci) => (
                <div key={col.id} className="flex-1 space-y-2">
                  ...
                  {Array.from({ length: col.count }, (_, ri) => {
                    ...
                    if (!hero) return (
                      <div key={ri} className="h-14 rounded-lg border border-dashed border-zinc-800 flex items-center justify-center text-[9px] text-zinc-700">Empty</div>
                    );
                    return (
                      <div key={ri} className={`p-2 rounded-lg border ${isAct ? 'border-fuchsia-700/50 bg-fuchsia-950/20' : 'border-zinc-800 bg-zinc-900 opacity-50'}`}>
                        ...
                        <div className="text-[8px] font-mono text-zinc-500">{calcPower(hero, targetLevel).toLocaleString()}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Range bars */}
            <div className="border-t border-zinc-800 pt-3 grid grid-cols-3 gap-3 text-xs">
              {[
                ...
                <div key={label} className="space-y-1">
                  <div className="flex justify-between text-[9px]"><span className="text-zinc-500">{label}</span><span className="font-mono text-zinc-400">{v}</span></div>
                  <div className="h-1 bg-zinc-800 rounded-full overflow-hidden"><div className={`h-full rounded-full ${c}`} style={{ width: `${(v / rangeCoverage.total) * 100}%` }} /></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <button onClick={() => {
              ...
              className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-xl border border-zinc-700 transition-colors">
              <Check size={14} /> Copy as Text
            </button>
          </div>
          {/* Text preview */}
          <div className="p-4 bg-zinc-950 border border-zinc-800 rounded-xl font-mono text-xs text-zinc-400 space-y-0.5">
            {ALL_SLOTS.map(sd => {
